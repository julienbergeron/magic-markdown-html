...
find /Users/julien/Documents/git_project_magic_html/example -type f -name "*.md" | while read i ; do FILE=$i; basename "$FILE"; chemin=$FILE; cd "${chemin%/*}"; f="$(basename -a $FILE)";SUBSTR=$(echo $f | cut -d'.' -f 1);pandoc  -s $i -f markdown-auto_identifiers --template /Users/julien/Documents/git_project_magic_html/template.html -t html -o $SUBSTR.html; done
